...
from app.admin.utils.datatable import DataTableWindow
...
class AdminWindow(BoxLayout, Screen):
    ...
    def update_user(self, first, last, user, pwd, des):
        ...
        if user == '':
            ...
            if user == None:
                ...
                res = self.users.update_one({'user_name': user['user_name']}, {
                    '$set': {'first_name': first, 'last_name': last, 'user_name': user['user_name'], 'password': pwd,
    ...
    def view_stats(self):
        ...
        csv = os.path.join(package_directory, 'products_purchase.csv')
